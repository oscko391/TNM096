...
def hill_climbing(problem):
    ...
    while True:
        ...
        if problem.value(neighbor.state) <= problem.value(current.state):
            break
        current = neighbor
    return current    # Return state in order to be compatible with simulated_annealing - kvlinden, 19dec2012
...
def simulated_annealing(problem, schedule=exp_schedule()):
    ...
    for t in xrange(sys.maxint):
        T = schedule(t)
        if T == 0:
            break  # modified (see below) - kvlinden
        neighbors = current.expand(problem)
        if not neighbors:
            break  # modified (see below) - kvlinden
        ...
        if delta_e > 0 or probability(math.exp(delta_e/T)):
            current = next
    return current # extracted return here to allow easier changing (see hill_climbing for fuller explanation of the problem) - kvlinden
...
class NQueensProblem(Problem):
    ...
    >>> from aima.search import depth_first_tree_search, NQueensProblem
